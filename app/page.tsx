...
const categories = [
  {
    ...
    label: 'Trailers',
    icon: '🚛',
    description: 'Utility, dump, car haulers & more',
  ...
  },
  {
    ...
    icon: '🚜',
    description: 'Mini excavators, skid steers, trenchers',
  ...
  },
  {
    ...
    label: 'Tools',
    icon: '🔧',
    description: 'Power tools, compactors, generators',
...
export default function HomePage() {
  ...
  useEffect(() => {
    setListings(getListings().filter((l) => l.available))
  }, [])
      ...
      <section className="bg-gradient-to-br from-brand-600 to-orange-700 text-white py-16 px-4">
        <div className="max-w-3xl mx-auto text-center">
          <h1 className="text-4xl sm:text-5xl font-bold leading-tight">
            Rent Equipment<br />From Your Neighbors
          </h1>
          <p className="mt-4 text-lg text-orange-100">
            Find trailers, backhoes, and tools available right in your area.
          </p>
          ...
          <form onSubmit={handleSearchSubmit} className="mt-8 flex gap-2 max-w-xl mx-auto">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3.5 rounded-xl text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-white/50 shadow-lg text-sm"
              />
              {query && (
                <button
                  type="button"
            ...
            <button
              type="submit"
              className="px-5 py-3.5 bg-white text-brand-600 font-semibold rounded-xl hover:bg-orange-50 transition-colors shadow-lg text-sm shrink-0"
            >
              Search
            </button>
          </form>
          ...
          <div className="mt-4">
            <Link href="/post" className="text-sm text-orange-200 hover:text-white underline underline-offset-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {categories.map((cat) => (
            <Link
              ...
              className={`flex items-center gap-4 p-5 rounded-2xl border-2 transition-all ${cat.color}`}
            >
