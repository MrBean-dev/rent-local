...
import { fetchMyRequests } from '@/lib/db'
import { useAuth } from '@/components/AuthProvider'
import type { RentalRequest } from '@/lib/types'
import { formatDate } from '@/lib/utils'

const statusStyles: Record<string, string> = {
  pending: 'bg-yellow-100 text-yellow-700', approved: 'bg-green-100 text-green-700', declined: 'bg-red-100 text-red-500',
}
const statusIcon: Record<string, string> = { pending: '⏳', approved: '✅', declined: '✕' }
...
export default function MyRequestsPage() {
  ...
  if (!user) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4 px-4 text-center">
      ...
      <p className="font-semibold text-gray-700">Sign in to see your requests</p>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex gap-1 mt-5 border-b border-white/20 -mb-10">
            {(['all', 'pending', 'approved', 'declined'] as const).map((s) => (
              <button key={s} onClick={() => setFilter(s)} className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-colors capitalize ${filter === s ? 'border-white text-white' : 'border-transparent text-orange-200 hover:text-white'}`}>
                {s === 'all' ? 'All' : s}
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 mt-14 pb-10">
        {visible.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm text-center py-20">
            ...
            <p className="font-semibold text-gray-700">{filter === 'all' ? "You haven't sent any requests yet." : `No ${filter} requests.`}</p>
            {filter === 'all' && <Link href="/listings" className="mt-4 inline-block text-sm text-brand-600 hover:underline">Browse equipment →</Link>}
          </div>
        ) : (
